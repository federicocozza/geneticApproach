library(Biobase)
load("../Data/all_exprs_data.RData")
C7 <- exprs(exprData.7c)

rownames(C7) <- gsub("_at","",rownames(C7))

library(annotables)

mapping <- as.data.frame(grch38[,c("entrez","ensgene")])

map2 <- mapping[mapping$ensgene %in%  rownames(C7),]

map2 <- map2[-which(map2$entrez %in% NA),]

C7 <- C7[map2$ensgene,]

rownames(C7) <- map2$entrez

labelsC7 <- pData(exprData.7c)
labelsC7 <- labelsC7[colnames(C7),]
labelsC7 <- labelsC7[-c(1:8,10:ncol(labelsC7))]
colnames(labelsC7) <- "class"

labelsC7[which(labelsC7 == "Healthy"),"class"] = 1
labelsC7[which(labelsC7 == "Infection"),"class"] = 2
labelsC7[which(labelsC7 == "Allergy"),"class"] = 3
labelsC7[which(labelsC7 == "Autoimmunity"),"class"] = 4

labelMapping <-  data.frame(OriginalLabel = c("Healthy","Infection","Allergy","Autoimmunity"),numberLabels = 1:4)

colnames(labelsC7) = "Classes"

library(SNFtool)
C7 <- t(standardNormalization(t(C7)))

source("R/enrichmentReactomeAndKegg.R")
enrichRes <- enrichmentReactomeAndKeggByList(rownames(C7))
...
allPathways <- getPathways(enrichRes,RNA = C7,minimumSizeOfPathway = minimumSizeOfPathway)

source("R/SVM.R")
pythonPath <- "/home/neuronelab/miniconda2/bin/python"
SVMResults <- SVM(pythonPath,allPathways,labelsC7,core=8,outer_fold = 3,inner_fold = 2)
...
dataset <- C7
reductionPathwaysRes <- reductionPathwaysSVM(pythonPath,SVMWeights,dataset,labelsC7,percentage = 0.8,core = 8, outer_fold = 3 ,inner_fold = 2)
...
permResults <- permutationTest(pythonPath,numberPermutation,accuraciesMatrix = accuraciesMatrixRed,SVMWeighs = reductionPathwaysRes$svmWeights,dataSet = dataset,labels = labelsC7,pValueCutOff = pValueCutOff,core= 16,outer_fold=3,inner_fold=2)
...
dir.create("C7")
dir.create("C7/AccuracyVsSizePathwaysPlots")
ggsave(permResults$accuracyVsPathwaysSizePlot,filename = paste("C7/AccuracyVsSizePathwaysPlots/AccuracyVSPathwaysSizeAll.png",sep=""), width = 14, height = 12)
...
WriteXLS::WriteXLS(mp,"C7/C7-AllPathwaysNOFS.xls")
...
WriteXLS::WriteXLS(SignPathways,"C7/C7-OnlySignificantlyPathwaysNOFS.xls")

C7_NOFS_HistPlot <- ggplot(SignPathways, aes(accuracyReduced)) + geom_histogram(aes(fill=class),col="black") + ggtitle("Pathways Accuracy") +
...
ggsave(C7_NOFS_HistPlot,filename = "C7/HistogramAccuracySignificantPathways.png")

dir.create("C7/RDATA")
save.image(file="C7/RDATA/C7-AFTER-PERMUTATION-TEST.RData")
...
for(cl in significantClass){
  ...
  ggsave(p,filename = paste("C7/AccuracyVsSizePathwaysPlots/AccuracyVSPathwaysSize-",cl,".png",sep=""), width = 14, height = 12)
}
...
for(i in 1:length(allRedPath)){
  ...
  prova <- as.data.frame(t(C7[genes,]))
  prova <- stack(prova)
  prova$class <- labelsC7$Classes
  ...
  ggsave(paste("C7/BoxPlot-C7-NOFS/",names(allRedPath)[i],".png",sep=""), plot = pp,width = 800, height = 1400,limitsize = FALSE,units = "mm")
...
for(i in combinations){
  classifyByGenesResult[[i]] <- classifyByGenesList(pythonPath,i,importantGenes[[i]],C7,labelsC7,core = 8, outer_fold = 3 ,inner_fold = 2)
}
...
write.table(ImportantGenesDF,"C7/ImportantGenes.xls",row.names = F)
...
library(ggplot2)
dir.create("C7/ImportantGenes-BOXPLOT")
for(i in combinations){
  classComb1 <- strsplit(i,split="vs")[[1]]
  
  prova <- as.data.frame(t(C7[importantGenes[[i]],]))
  if(length(importantGenes[[i]])==1){
    ...
    ggsave(paste("C7/ImportantGenes-BOXPLOT/ImportantGenes-",i,".png",sep=""), plot = pp,width = 800, height = 1400,limitsize = FALSE,units = "mm")
  }else{
    prova <- stack(prova)
    prova$class <- labelsC7$Classes
    prova <- prova[c(which(prova$class == classComb1[1]),which(prova$class == classComb1[2])),]
...
C7Norm <- range01(C7[allImportantGenes,])

library(gplots)

dir.create("C7/ImportantGenes-HeatMap")
for(i in 1:length(importantGenes)){
  ...
  if(length(importantGenes[[i]]) == 1){
    # X1 <- as.matrix(t(C7Norm[importantGenes[[i]],which(labelsC7$Classes== strsplit(a,split="vs")[[1]][1])]))
    # X2 <- as.matrix(t(C7Norm[importantGenes[[i]],which(labelsC7$Classes==strsplit(a,split="vs")[[1]][2])]))
    # heatData <- cbind(X1,X2)
    # rownames(heatData) <- importantGenes[[i]]
  }else{
    heatData <- cbind(C7Norm[importantGenes[[i]],which(labelsC7$Classes== strsplit(a,split="vs")[[1]][1])],C7Norm[importantGenes[[i]],which(labelsC7$Classes==strsplit(a,split="vs")[[1]][2])])
    png(file = paste("C7/ImportantGenes-HeatMap/",names(importantGenes)[i],".png",sep=""),width = 800,height = 600)
    heatmap.2(heatData,Colv = FALSE,dendrogram = "row",trace = "none",colsep=dim(C7Norm[importantGenes[[i]],which(labelsC7$Classes==strsplit(a,split="vs")[[1]][1])])[2],sepcolor ="black",main=title,xlab="Patients",ylab= "Genes",labCol = F)
    dev.off()
  }
}
...
dir.create("C7/Statistiche")
dir.create("C7/HeatMapPathway")
for(i in 1:length(allRedPath)){
  ### statistiche (media varianza)
  classComb1 <- strsplit(strsplit(names(SVMWeightsOnlySign[names(allRedPath[i])]),split="-")[[1]][2],split="vs")[[1]]
  genes <- names(SVMWeightsOnlySign[[names(allRedPath[i])]])
  pathway <- as.data.frame(C7[genes,])
  idxCB1 <- which(labelsC7$Classes == classComb1[1])
  idxCB2 <- which(labelsC7$Classes == classComb1[2])
  ...
  write.table(dataStat, file= paste("C7/Statistiche/",names(allRedPath[i]),".csv",sep=""),row.names = F)
  ...
  ### creazione heatmap
  jpeg(file = paste("C7/HeatMapPathway/",names(allRedPath[i]),".jpeg",sep=""),width=1400,height=1200)
...
write.table(PathStatData,file="C7/StatisticheRiassuntive.csv",row.names = F)
...
for(i in 1:length(allRedPath2)){
  ...
  pp <- C7[genes,]
  pp1 <- pp[,which(labelsC7 == "1")]
  pp2 <- pp[,which(labelsC7 == "2")]
  pp3 <- pp[,which(labelsC7 == "3")]
  pp4 <- pp[,which(labelsC7 == "4")]
...
dev.off()
jpeg(file = paste("C7/HeatMap-PathwaysRelation.jpeg",sep=""),width=1500,height=1200)
...
pathCombProbabilities <- pathwaysCombination(pythonPath,probabilities,significantlyPathways,labelsClass = labelsC7,core=32,outer_fold=3,inner_fold=2,maxNumCombination = 2)
...
ggsave("C7/PathwayCombinationHistogram.png", plot = pathCombPlot,width = 800, height = 600,limitsize = FALSE,units = "mm")
...
WriteXLS::WriteXLS(allCombinationBest,ExcelFileName = "C7/allCombinationBest.xls",row.names = F)

save.image(file="C7/RDATA/C7-NO-FS-AfterPathwayCombination.RData")
...
metaFeatures <- matrix(0,nrow = length(rp),ncol=nrow(labelsC7))
rownames(metaFeatures) <- names(rp)
colnames(metaFeatures) <- rownames(labelsC7)

for(i in 1:length(rp)){
  metaPathway <- matrix(0,length(rp[[i]]),nrow(labelsC7))
  for(j in 1:length(rp[[i]])){
    ggg <- names(rp[[i]])[j]
    www <- (rp[[i]])[j]
    metaPathway[j,] <- www*C7[ggg,]
  }
...
for(comb in combinations){
  ...
  cols <- c(which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][1]),which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][2]))
  ...
  labels2classes <- labelsC7[cols,"Classes"]
  labels2classes <- as.data.frame(labels2classes)
  rownames(labels2classes) <- rownames(labelsC7)[cols]
...
combinations <- as.character(unique(permResults$significantlyPathways$classes))
for(comb in combinations){
  ncols <- sum(c(labelsC7$Classes == strsplit(comb,split="vs")[[1]][1],labelsC7$Classes == strsplit(comb,split="vs")[[1]][2]))
  ...
  for(i in 1:length(rpm)){
    if(grepl(comb,names(rpm)[i])){
      cols <- c(which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][1]),which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][2]))
      dataToFeatures <- C7[names(rpm[[i]]),cols]
...
maxCorGenes <- list()
for(comb in combinations){
  ncols[[comb]] <- c(which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][1]),which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][2]))
  maxCorGenes[[comb]] <- list()
  for(i in 1:length(rpm)){
    if(grepl(comb,names(rpm)[i])){
      matrixToAdd <- C7[names(rpm[[i]]),ncols[[1]]]
...
for(comb in combinations){
  ...
  temporanyDirectory_res <- paste(temporanyDirectory,"_res",sep="")
  dir.create(temporanyDirectory)
  corClasses <- C7[unlist(maxCorGenes[[comb]]),ncols[[comb]]]
...
write.table(corDataWithGenes,file="C7/CorrelatioFeaturesData.csv",row.names = F)
...
for(i in 1:length(rpm)){
  ...
  patientsClass1 <- which(labelsC7$Classes == strsplit(classType,split="vs")[[1]][1])
  patientsClass2 <- which(labelsC7$Classes == strsplit(classType,split="vs")[[1]][2])
  
  patientsIdx <- c(patientsClass1,patientsClass2)
  
  matrixPathway <- C7[names(rpm[[i]]),patientsIdx]
...
for(i in combinations){
  allGenesResults[[i]] <- classifyByGenesList(pythonPath,i,rownames(C7),C7,labelsC7,core = 8, outer_fold = 3 ,inner_fold = 2)
}
...
library(WriteXLS)
WriteXLS(as.data.frame(allRes),ExcelFileName ="C7/FinalClassification.xls")
...
ggsave(finalClassificationPlot ,filename = paste("C7/FinalClassificationHistogram.png",sep=""), width = 14, height = 12)

save.image(file="C7/RDATA/C7-NO-FS-DopoConfrontoClassificazione.RData")
...
WriteXLS::WriteXLS(SignPathways,"C7/C7-SignificantlyPathwaysWithGenesCTD.xls")
...
for(comb in combinations){
  ...
  for(i in 1:(nrow(pathComb)-2)){
    ...
    for(j in 1:(nrow(pathCombMin))){
      ...
      labelPath <- paste(temporanyDirectory,"/",path,"/labels",sep="")
      idx <- c(which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][1]), which(labelsC7$Classes == strsplit(comb,split="vs")[[1]][2]))
      labels2Classes <- labelsC7[idx,"Classes"]
      labels2Classes <- as.data.frame(labels2Classes)
      rownames(labels2Classes) <- rownames(labelsC7)[idx]
      colnames(labels2Classes) <- "x"
      write.table(labels2Classes,file = labelPath ,quote=F)
    }
...
for(i in 1:length(RFEFinalResults)){
  ...
  ggsave(RFEPlot,filename = paste("C7/RFE/plots/",names(RFEFinalResults)[i],".png",sep=""), width = 14, height = 12)
  
  write.table(RFEFinalResults[[i]],paste("C7/RFE/",names(RFEFinalResults)[i],".xls"),row.names = F)
}
...
ggsave(RFEPlotAll,filename = paste("C7/RFE/plots/AllCombinations.png",sep=""), width = 14, height = 12)

save.image("C7/RDATA/C7-AFTER-RFE.RData")
