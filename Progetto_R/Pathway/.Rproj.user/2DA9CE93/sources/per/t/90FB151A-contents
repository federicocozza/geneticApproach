...
labelMapping <-  data.frame(OriginalLabel = c("SCC1","SCC2","AC1","AC2"),numberLabels = 1:4)

colnames(labelsLC4) = "Classes"

library(SNFtool)
lc4 <- t(standardNormalization(t(lc4)))
...
SVMResults <- SVM(pythonPath,allPathways,labelsLC4,core=8,outer_fold = 3,inner_fold = 2)
...
dataset <- lc4
reductionPathwaysRes <- reductionPathwaysSVM(pythonPath,SVMWeights,dataset,labelsLC4,percentage = 0.8,core = 8, outer_fold = 3 ,inner_fold = 2)
...
permResults <- permutationTest(pythonPath,numberPermutation,accuraciesMatrix = accuraciesMatrixRed,SVMWeighs = reductionPathwaysRes$svmWeights,dataSet = dataset,labels = labelsLC4,pValueCutOff = pValueCutOff,core= 16,outer_fold=3,inner_fold=2)
...
dir.create("LC4")
dir.create("LC4/AccuracyVsSizePathwaysPlots")
ggsave(permResults$accuracyVsPathwaysSizePlot,filename = paste("LC4/AccuracyVsSizePathwaysPlots/AccuracyVSPathwaysSizeAll.png",sep=""), width = 14, height = 12)
...
WriteXLS::WriteXLS(mp,"LC4/LC4-AllPathwaysNOFS.xls")
...
WriteXLS::WriteXLS(SignPathways,"LC4/LC4-OnlySignificantlyPathwaysNOFS.xls")

LC4_NOFS_HistPlot <- ggplot(SignPathways, aes(accuracyReduced)) + geom_histogram(aes(fill=class),col="black") + ggtitle("Pathways Accuracy") +
...
ggsave(LC4_NOFS_HistPlot,filename = "LC4/HistogramAccuracySignificantPathways.png")

dir.create("LC4/RDATA")
save.image(file="LC4/RDATA/LC4-AFTER-PERMUTATION-TEST.RData")
...
for(cl in significantClass){
  ...
  ggsave(p,filename = paste("LC4/AccuracyVsSizePathwaysPlots/AccuracyVSPathwaysSize-",cl,".png",sep=""), width = 14, height = 12)
}
...
dir.create("LC4/BoxPlot-LC4-NOFS")
...
for(i in 1:length(allRedPath)){
  ...
  prova <- as.data.frame(t(lc4[genes,]))
  prova <- stack(prova)
  prova$class <- labelsLC4$Classes
  ...
  ggsave(paste("LC4/BoxPlot-LC4-NOFS/",names(allRedPath)[i],".png",sep=""), plot = pp,width = 800, height = 1400,limitsize = FALSE,units = "mm")
...
for(i in combinations){
  classifyByGenesResult[[i]] <- classifyByGenesList(pythonPath,i,importantGenes[[i]],lc4,labelsLC4,core = 8, outer_fold = 3 ,inner_fold = 2)
}
...
write.table(ImportantGenesDF,"LC4/ImportantGenes.xls",row.names = F)
...
library(ggplot2)
dir.create("LC4/ImportantGenes-BOXPLOT")
for(i in combinations){
  classComb1 <- strsplit(i,split="vs")[[1]]
  
  prova <- as.data.frame(t(lc4[importantGenes[[i]],]))
  if(length(importantGenes[[i]])==1){
    ...
    ggsave(paste("LC4/ImportantGenes-BOXPLOT/ImportantGenes-",i,".png",sep=""), plot = pp,width = 800, height = 1400,limitsize = FALSE,units = "mm")
  }else{
  prova <- stack(prova)
  prova$class <- labelsLC4$Classes
  prova <- prova[c(which(prova$class == classComb1[1]),which(prova$class == classComb1[2])),]
...
lc4Norm <- range01(lc4[allImportantGenes,])

library(gplots)

dir.create("LC4/ImportantGenes-HeatMap")
for(i in 1:length(importantGenes)){
  ...
  if(length(importantGenes[[i]]) == 1){
    # X1 <- as.matrix(t(lc4Norm[importantGenes[[i]],which(labelsLC4$Classes== strsplit(a,split="vs")[[1]][1])]))
    # X2 <- as.matrix(t(lc4Norm[importantGenes[[i]],which(labelsLC4$Classes==strsplit(a,split="vs")[[1]][2])]))
    # heatData <- cbind(X1,X2)
    # rownames(heatData) <- importantGenes[[i]]
  }else{
    heatData <- cbind(lc4Norm[importantGenes[[i]],which(labelsLC4$Classes== strsplit(a,split="vs")[[1]][1])],lc4Norm[importantGenes[[i]],which(labelsLC4$Classes==strsplit(a,split="vs")[[1]][2])])
    png(file = paste("LC4/ImportantGenes-HeatMap/",names(importantGenes)[i],".png",sep=""),width = 800,height = 600)
    heatmap.2(heatData,Colv = FALSE,dendrogram = "row",trace = "none",colsep=dim(lc4Norm[importantGenes[[i]],which(labelsLC4$Classes==strsplit(a,split="vs")[[1]][1])])[2],sepcolor ="black",main=title,xlab="Patients",ylab= "Genes",labCol = F)
    dev.off()
  }
}
...
dir.create("LC4/Statistiche")
dir.create("LC4/HeatMapPathway")
for(i in 1:length(allRedPath)){
  ### statistiche (media varianza)
  classComb1 <- strsplit(strsplit(names(SVMWeightsOnlySign[names(allRedPath[i])]),split="-")[[1]][2],split="vs")[[1]]
  genes <- names(SVMWeightsOnlySign[[names(allRedPath[i])]])
  pathway <- as.data.frame(lc4[genes,])
  idxCB1 <- which(labelsLC4$Classes == classComb1[1])
  idxCB2 <- which(labelsLC4$Classes == classComb1[2])
  ...
  write.table(dataStat, file= paste("LC4/Statistiche/",names(allRedPath[i]),".csv",sep=""),row.names = F)
  ...
  ### creazione heatmap
  jpeg(file = paste("LC4/HeatMapPathway/",names(allRedPath[i]),".jpeg",sep=""),width=1400,height=1200)
...
write.table(PathStatData,file="LC4/StatisticheRiassuntive.csv",row.names = F)
...
for(i in 1:length(allRedPath2)){
  ...
  pp <- lc4[genes,]
  pp1 <- pp[,which(labelsLC4 == "1")]
  pp2 <- pp[,which(labelsLC4 == "2")]
  pp3 <- pp[,which(labelsLC4 == "3")]
  pp4 <- pp[,which(labelsLC4 == "4")]
...
dev.off()
jpeg(file = paste("LC4/HeatMap-PathwaysRelation.jpeg",sep=""),width=1500,height=1200)
...
pathCombProbabilities <- pathwaysCombination(pythonPath,probabilities,significantlyPathways,labelsClass = labelsLC4,core=32,outer_fold=3,inner_fold=2,maxNumCombination = 2)
...
ggsave("LC4/PathwayCombinationHistogram.png", plot = pathCombPlot,width = 800, height = 600,limitsize = FALSE,units = "mm")
...
WriteXLS::WriteXLS(allCombinationBest,ExcelFileName = "LC4/allCombinationBest.xls",row.names = F)

save.image(file="LC4/RDATA/LC4-NO-FS-AfterPathwayCombination.RData")
...
metaFeatures <- matrix(0,nrow = length(rp),ncol=nrow(labelsLC4))
rownames(metaFeatures) <- names(rp)
colnames(metaFeatures) <- rownames(labelsLC4)

for(i in 1:length(rp)){
  metaPathway <- matrix(0,length(rp[[i]]),nrow(labelsLC4))
  for(j in 1:length(rp[[i]])){
    ggg <- names(rp[[i]])[j]
    www <- (rp[[i]])[j]
    metaPathway[j,] <- www*lc4[ggg,]
  }
...
for(comb in combinations){
  ...
  cols <- c(which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][1]),which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][2]))
  ...
  labels2classes <- labelsLC4[cols,"Classes"]
  labels2classes <- as.data.frame(labels2classes)
  rownames(labels2classes) <- rownames(labelsLC4)[cols]
...
combinations <- as.character(unique(permResults$significantlyPathways$classes))
for(comb in combinations){
  ncols <- sum(c(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][1],labelsLC4$Classes == strsplit(comb,split="vs")[[1]][2]))
  ...
  for(i in 1:length(rpm)){
    if(grepl(comb,names(rpm)[i])){
      cols <- c(which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][1]),which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][2]))
      dataToFeatures <- lc4[names(rpm[[i]]),cols]
...
maxCorGenes <- list()
for(comb in combinations){
  ncols[[comb]] <- c(which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][1]),which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][2]))
  maxCorGenes[[comb]] <- list()
  for(i in 1:length(rpm)){
    if(grepl(comb,names(rpm)[i])){
      matrixToAdd <- lc4[names(rpm[[i]]),ncols[[1]]]
...
for(comb in combinations){
  ...
  temporanyDirectory_res <- paste(temporanyDirectory,"_res",sep="")
  dir.create(temporanyDirectory)
  corClasses <- lc4[unlist(maxCorGenes[[comb]]),ncols[[comb]]]
...
write.table(corDataWithGenes,file="LC4/CorrelatioFeaturesData.csv",row.names = F)
...
for(i in 1:length(rpm)){
  ...
  patientsClass1 <- which(labelsLC4$Classes == strsplit(classType,split="vs")[[1]][1])
  patientsClass2 <- which(labelsLC4$Classes == strsplit(classType,split="vs")[[1]][2])
  
  patientsIdx <- c(patientsClass1,patientsClass2)
  
  matrixPathway <- lc4[names(rpm[[i]]),patientsIdx]
...
for(i in combinations){
  allGenesResults[[i]] <- classifyByGenesList(pythonPath,i,rownames(lc4),lc4,labelsLC4,core = 8, outer_fold = 3 ,inner_fold = 2)
}
...
library(WriteXLS)
WriteXLS(as.data.frame(allRes),ExcelFileName ="LC4/FinalClassification.xls")
...
ggsave(finalClassificationPlot ,filename = paste("LC4/FinalClassificationHistogram.png",sep=""), width = 14, height = 12)

save.image(file="LC4/RDATA/LC4-NO-FS-DopoConfrontoClassificazione.RData")
...
WriteXLS::WriteXLS(SignPathways,"LC4/LC4-SignificantlyPathwaysWithGenesCTD.xls")
...
for(comb in combinations){
  ...
  for(i in 1:(nrow(pathComb)-2)){
    ...
    for(j in 1:(nrow(pathCombMin))){
      ...
      labelPath <- paste(temporanyDirectory,"/",path,"/labels",sep="")
      idx <- c(which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][1]), which(labelsLC4$Classes == strsplit(comb,split="vs")[[1]][2]))
      labels2Classes <- labelsLC4[idx,"Classes"]
      labels2Classes <- as.data.frame(labels2Classes)
      rownames(labels2Classes) <- rownames(labelsLC4)[idx]
      colnames(labels2Classes) <- "x"
      write.table(labels2Classes,file = labelPath ,quote=F)
    }
...
for(i in 1:length(RFEFinalResults)){
  ...
  ggsave(RFEPlot,filename = paste("LC4/RFE/plots/",names(RFEFinalResults)[i],".png",sep=""), width = 14, height = 12)
  
  write.table(RFEFinalResults[[i]],paste("LC4/RFE/",names(RFEFinalResults)[i],".xls"),row.names = F)
}
...
ggsave(RFEPlotAll,filename = paste("LC4/RFE/plots/AllCombinations.png",sep=""), width = 14, height = 12)

save.image("LC4/RDATA/LC4-AFTER-RFE.RData")
